...
// // console.log('MenuReducer Called', state, action)
    ...
    // // console.log('MenuReducer Called menudef entry is ', menuEntry)
    ...
    if (menuEntryAll !== undefined) {
        ...
        if (state.DeferedActionsList.indexOf(action.type) !== -1) {
            // // console.log('MenuReducedr action is in defered list, check for menu reducers and remove entry from defered list')
                ...
                if (menuEntryAll !== undefined) {
                    menuEntryAll.map((menuEntry) => {
                            if (menuEntry.actionName === action.type || (Array.isArray(menuEntry.actionNames) && menuEntry.actionNames.indexOf(action.type) !== -1)) {
                                ...
                                state = update(state, {MenuData: {[menuEntry.area]: {[menuEntry.subArea]: {items: {$set: newLocalState}}}}})
            ...
            state = update(state, {DeferedActionsList: {$splice: [[state.DeferedActionsList.indexOf(action.type), 1]]}})
        }
        return state
    }
}
