...
const getInitialStageStatus = (StartupStageDefinition) => {
    var obj={}
    Object.keys(StartupStageDefinition).sort().map((StartupStageDefinitionName, index) => {
...
const updateStatus = (state)=> {
        ...
        if (totalCount === successCount) {
            // set next stage
            state = update(state, {status: {stagePercentage: {$set: 100}}}) //epic will start next stage or report finish
...
const stepReducer = (state, action) => {
    ...
    currentStage.some((stepDef) => {
        ...
        if (action.type === stepDef.actionNameSuccess) {
            state = update(state, {stepStatus: {[stepDef.name]: {$set: LoadingStateEnum.SUCCESS}}})
            // increase step for every success
            state = update(state, {status: {totalFinishedSteps: {$set: state.status.totalFinishedSteps + 1}}})
            //update count in stageStatus
            state = update(state, {stageStatus: {[currentStageIndex]:{successCount: {$set: state.stageStatus[currentStageIndex].successCount + 1}}}})
            ...
            state = update(state, {stepStatus: {[stepDef.name]: {$set: LoadingStateEnum.FAILURE}}})
            //update count in stageStatus
            if (stepDef.required === true) {
                state = update(state, {stageStatus: {[currentStageIndex]:{failureCount: {$set: state.stageStatus[currentStageIndex].failureCount + 1}}}})
            } else {
                state = update(state, {status: {totalFinishedSteps: {$set: state.status.totalFinishedSteps + 1}}})
                state = update(state, {stageStatus: {[currentStageIndex]:{successCount: {$set: state.stageStatus[currentStageIndex].successCount + 1}}}})
            }
